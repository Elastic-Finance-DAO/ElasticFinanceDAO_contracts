...
contract TokenGeyser is IStaking, Ownable {
    ...
    event ScheduleRebaseReward(uint256 id, uint256 bonusShares);
    ...
    struct UnlockSchedule {
        ...
        uint256 endAtSec;
        uint256 durationSec;
        uint256 positiveRebaseBonus;
        uint256 negativeRebaseBonus;
        int256 deltaCounter; //incremented and decremented based on rebases
    }
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
                ...
                address AMPLContractAddress) public {
    ...
    * Else it will increment or decrement (depending on the rebase being positive or negative)
    * the schedule delta counter for schedules with rebase bonuses
    * then trigger a token unlock
    */
    function rewardRebase() public {
        ...
        for(uint256 i = 0; i < unlockSchedules.length; i++) {
            UnlockSchedule storage schedule = unlockSchedules[i];
            if(schedule.positiveRebaseBonus > 0 || schedule.negativeRebaseBonus > 0) {
                schedule.deltaCounter = newTotalSupply > lastAMPLTotalSupply? schedule.deltaCounter + 1 :  schedule.deltaCounter - 1;
            }
        }
     ...
     * @param bonusPositiveRebase Bonus to unlock during positive rebase
     * @param bonusNegativeRebase Bonus to unlock during negative rebase
     */
    function lockTokens(uint256 amount, uint256 durationSec, uint256 bonusPositiveRebase, uint256 bonusNegativeRebase) external onlyOwner {
        ...
        schedule.positiveRebaseBonus = bonusPositiveRebase;
        schedule.negativeRebaseBonus = bonusNegativeRebase;
        schedule.deltaCounter = 0;
    ...
    function unlockScheduleShares(uint256 s) private returns (uint256) {
        ...
        // if we have rebase bonus enabled for this schedule
        if(schedule.positiveRebaseBonus > 0 || schedule.negativeRebaseBonus > 0) {
            //compute which bonus to apply based on delta
            uint256 rebaseBonus = schedule.deltaCounter > 0? schedule.positiveRebaseBonus : schedule.negativeRebaseBonus;
            uint256 absDelta = schedule.deltaCounter < 0? uint256(-schedule.deltaCounter) : uint256(schedule.deltaCounter);
            //compute the right amount of unlocked shares by applying the rebase bonus % for every delta
            uint256 unlockBonusShares = schedule.initialLockedShares.mul(rebaseBonus.mul(absDelta)).div(10**REBASE_BONUS_DECIMALS);
            //add the bonus shares to the shares to unlock
            sharesToUnlock = sharesToUnlock.add(unlockBonusShares);
            // make sure we're not trying to unlock more shares than left in the schedule
            uint256 remains = schedule.initialLockedShares.sub(schedule.unlockedShares);
            if(sharesToUnlock > remains) {
                //just give all we have left
                sharesToUnlock = remains;
            }
            //reset the delta counter
            schedule.deltaCounter = 0;
            emit ScheduleRebaseReward(s, unlockBonusShares);
        }
